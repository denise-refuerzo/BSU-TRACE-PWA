...
export default function OfficeSubmissionsTab({officeId,onProcessed=()=>{}}) {
  ...
  const eligible=process && Number(process.resolved_origin_office_id)===Number(officeId);
  ...
  const submit=async e=>{
    ...
    try {
      const res=await fetchWithAuth('/api/documents',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title:form.title,processTypeId:Number(form.processTypeId),edc,completeOriginProcessing:Boolean(eligible && form.completeOriginProcessing)})});
  ...
  return <div className="max-w-6xl mx-auto space-y-5 text-left">
    ...
    {showModal&&<DocumentSubmissionModal setShowModal={setShowModal} submitDocument={submit} form={form} setForm={setForm} handleProcessChange={id=>setForm({...form,processTypeId:id,completeOriginProcessing:false})} processTypes={processTypes} workflowsLoading={loading} workflowError={workflowError} retryWorkflows={()=>{setLoading(true);workflows();}} estimatedDate={estimate?formatPhilippineDate(estimate):"Estimate unavailable"} selectedRoutePreview={process?.resolved_route_names || []} canCompleteOriginProcessing={eligible} submitting={busy} submissionError={error}/>}
