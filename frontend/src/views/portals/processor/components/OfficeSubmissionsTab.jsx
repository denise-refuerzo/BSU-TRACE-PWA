...
export default function OfficeSubmissionsTab({ officeId, processTypes: providedProcessTypes = [], onProcessed = () => {}, onOpenChat }) {
  ...
  const getStatusStyles = (status) => {
    ...
    if (s.includes('completed') || s.includes('finalized')) return 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800';
    if (s.includes('cancelled')) return 'bg-neutral-100 dark:bg-gray-800 text-neutral-700 dark:text-gray-300 border border-neutral-300 dark:border-gray-700';
    if (s.includes('action required') || s.includes('halted')) return 'bg-red-50 dark:bg-red-900/30 text-[#D32F2F] dark:text-red-400 border border-red-200 dark:border-red-800';
  ...
  return (
    <div className="space-y-6 max-w-8xl mx-auto text-left animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">Manage and track only the documents you submitted.</p>
        <button 
      ...
      {error && !showModal && (
        <div className="p-3 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900 text-red-800 dark:text-red-300 text-xs font-bold rounded-xl flex items-center gap-2 shadow-sm">
          <AlertCircle size={16} className="shrink-0" /> {error}
        </div>
      )}

      {/* MATRIX TABLE CONTAINER */}
      <div className="bg-white dark:bg-[#180e10] border border-gray-200 dark:border-[#42292f] rounded-2xl shadow-sm overflow-hidden flex flex-col scroll-mt-6">
        
        {/* Table Controls Header */}
        <div className="p-5 border-b border-gray-100 dark:border-[#42292f] flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 bg-gray-50/50 dark:bg-[#1c1113]">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            {/* Filter Dropdown */}
            <div className="flex items-center gap-1.5 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 bg-white dark:bg-[#1c1113] shadow-sm focus-within:ring-1 focus-within:ring-[#D32F2F] transition-all cursor-pointer w-full sm:w-auto">
              <Filter size={14} className="text-gray-400 shrink-0" />
              <select 
                value={filterStatus} 
                onChange={e => setFilterStatus(e.target.value)} 
                className="bg-transparent text-xs outline-none cursor-pointer font-medium text-gray-700 dark:text-gray-300 appearance-none w-full pr-2"
              >
            ...
            <div className="relative w-full sm:w-56">
              ...
              <input 
                ...
                className="w-full pl-9 pr-4 py-2 text-xs border border-gray-300 dark:border-gray-700 rounded-lg outline-none focus:ring-1 focus:ring-[#D32F2F] bg-white dark:bg-[#1c1113] text-gray-900 dark:text-white shadow-sm transition-all" 
              />
            </div>
          </div>
        </div>
              ...
              {isDocsLoading && documents.length === 0 ? (
                  ...
                  <tr key={`skeleton-${i}`}>
                    <td className="p-4 align-middle">
                      <div className="flex min-w-0 items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-gray-200 dark:bg-gray-800 animate-pulse shrink-0"></div>
                    ...
                    <td className="p-4"><div className="h-6 w-24 bg-gray-200 dark:bg-gray-800 rounded animate-pulse"></div></td>
                    <td className="p-4"><div className="h-4 w-32 bg-gray-200 dark:bg-gray-800 rounded animate-pulse"></div></td>
                    <td className="p-4"><div className="h-4 w-24 bg-gray-200 dark:bg-gray-800 rounded animate-pulse"></div></td>
                    <td className="p-4"><div className="h-6 w-24 bg-gray-200 dark:bg-gray-800 rounded-md animate-pulse"></div></td>
                    <td className="p-4 text-center"><div className="h-8 w-8 bg-gray-200 dark:bg-gray-800 rounded-lg animate-pulse mx-auto"></div></td>
                  </tr>
                ))
                ...
                filteredDocs.map(d => (
                  <tr key={d.ini_id} 
                      ...
                      className={`transition-colors cursor-pointer group ${selected?.ini_id === d.ini_id ? 'bg-red-50/40 dark:bg-red-900/10' : 'hover:bg-gray-50/80 dark:hover:bg-[#2b1317]/50'}`}>
                    <td className="p-4 align-middle">
                      <div className="flex min-w-0 items-center gap-3">
                        <div className={`p-2 rounded-lg transition-all duration-150 shrink-0 ${selected?.ini_id === d.ini_id ? 'bg-white dark:bg-[#180e10] shadow-sm border border-red-100 dark:border-red-900' : 'bg-gray-50 dark:bg-[#1c1113] border border-gray-100 dark:border-gray-800'}`}>
                          <FileText size={16} className={`transition-colors duration-150 ${selected?.ini_id === d.ini_id ? 'text-[#D32F2F] dark:text-red-400' : 'text-gray-500 dark:text-gray-400'}`} />
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="truncate font-bold text-gray-900 dark:text-white text-sm leading-5" title={d.title}>{d.title}</p>
                          <p className="mt-1 whitespace-nowrap text-[10px] font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">
                    ...
                    <td className="p-4 align-middle">
                      <span title={d.qr_code} className="block max-w-full truncate rounded border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#1c1113] px-2 py-1 font-mono text-xs font-bold text-gray-600 dark:text-gray-300">
                        {d.qr_code}
                      </span>
                    </td>
                    <td className="p-4 align-middle">
                      <span className="flex items-start gap-1.5 text-xs font-medium leading-5 text-gray-700 dark:text-gray-300">
                    ...
                    <td className="p-4 align-middle text-xs text-gray-600 dark:text-gray-400 font-medium whitespace-nowrap">
                    ...
                    <td className="p-4 text-center align-middle">
                      <button 
                        ...
                        className="p-2 rounded-lg hover:bg-white dark:hover:bg-gray-800 border border-transparent hover:border-gray-200 dark:hover:border-gray-700 hover:shadow-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white mx-auto flex items-center justify-center transition-all focus:outline-none cursor-pointer"
                      >
                        <MoreVertical size={16} />
                      </button>
                      
                      {d.status?.toLowerCase() === 'action required' && (
                        <div className="mt-3 w-full rounded-xl border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-900/20 p-3 text-left" onClick={e => e.stopPropagation()}>
                          <p className="text-[11px] text-red-800 dark:text-red-300 font-medium mb-2 leading-relaxed">
                          ...
                          {d.release_time ? (
                            <button 
                              ...
                              className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-red-700 dark:text-red-400 hover:text-red-900 cursor-pointer mt-2 transform active:scale-95 transition-[transform,colors] duration-200"
                            >
                ...
                <tr>
                  <td colSpan="6" className="p-12 text-center bg-gray-50/50 dark:bg-[#1c1113]">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-sm font-bold text-gray-600 dark:text-gray-400">No submissions found</p>
      ...
      {revision && (
        <div className="fixed inset-0 bg-neutral-950/60 backdrop-blur-xs z-[100] p-4 flex items-center justify-center animate-in fade-in duration-150">
          <form onSubmit={resubmit} className="bg-white dark:bg-[#180e10] border border-gray-200 dark:border-[#42292f] rounded-2xl shadow-2xl max-w-md w-full mx-auto overflow-hidden animate-in zoom-in-95 duration-200 text-left">
            ...
            <div className="p-6 space-y-5">
              <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900 p-3 rounded-xl">
                <p className="text-[10px] font-black uppercase text-red-800 dark:text-red-400 tracking-wider mb-1">Required Action</p>
                <p className="text-xs text-red-700 dark:text-red-300 font-medium">{revision.last_action}</p>
              </div>

              <div>
                <label className="block text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase mb-1 tracking-wide">Document Title</label>
                <input 
                  required 
                  maxLength={150} 
                  value={revision.title} 
                  onChange={e => setRevision({ ...revision, title: e.target.value })} 
                  className="w-full px-4 py-2 text-xs border border-gray-300 dark:border-gray-700 rounded-lg outline-none focus:ring-1 focus:ring-[#D32F2F] bg-gray-50 dark:bg-[#1c1113] font-bold text-gray-900 dark:text-white transition-all duration-200"
                />
              </div>
              ...
              <label className="flex items-start gap-3 cursor-pointer group">
                <div className="relative flex items-center mt-0.5 shrink-0">
                  ...
                  <div className="w-4 h-4 border-2 border-gray-300 dark:border-gray-700 rounded flex items-center justify-center peer-checked:bg-[#D32F2F] peer-checked:border-[#D32F2F] transition-colors duration-150">
                ...
                <span className="text-xs font-medium text-gray-600 dark:text-gray-300 leading-snug group-hover:text-gray-900 dark:group-hover:text-white transition-colors duration-150">
                  I confirm that I have corrected the physical document and its supporting materials according to the remarks above.
                </span>
              </label>

              {error && <p role="alert" className="text-xs font-bold text-red-600 dark:text-red-400 flex items-center gap-1"><AlertCircle size={14} className="shrink-0"/> {error}</p>}
            </div>

            <div className="p-4 bg-gray-50 dark:bg-[#1c1113] border-t border-gray-100 dark:border-[#42292f] flex justify-end gap-2">
              <button disabled={busy} type="button" onClick={() => setRevision(null)} className="px-5 py-2 bg-white dark:bg-[#180e10] border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 text-xs font-bold rounded-lg shadow-sm cursor-pointer transform active:scale-95 transition-all duration-200">
                Cancel
              </button>
              <button disabled={busy} type="submit" className="px-6 py-2 bg-[#D32F2F] dark:bg-red-800 hover:bg-[#b71c1c] dark:hover:bg-red-700 text-white text-xs font-bold uppercase tracking-wider rounded-lg shadow-md cursor-pointer disabled:opacity-50 transform active:scale-95 hover:-translate-y-0.5 transition-all duration-200">
