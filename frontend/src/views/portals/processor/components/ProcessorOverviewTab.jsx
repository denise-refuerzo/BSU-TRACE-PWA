...
export default function ProcessorOverviewTab({
  ...
  const kanbanColumns = [
    { id: 'Awaiting Scan-In', label: 'Awaiting Scan-In', count: awaitingScanInCount, color: 'border-t-[#D32F2F] dark:border-t-red-500', text: 'text-[#D32F2F] dark:text-red-400', bg: 'bg-red-50 dark:bg-red-900/20', icon: <Scan size={16} /> },
    { id: 'Pending', label: 'Pending Docs', count: pendingCount, color: 'border-t-amber-500', text: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-900/20', icon: <Clock size={16} /> },
    { id: 'In Verification', label: 'In Verification', count: inVerificationCount, color: 'border-t-purple-500', text: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-900/20', icon: <Scale size={16} /> },
    { id: 'Signed', label: 'Ready for Release', count: signedCount, color: 'border-t-blue-600 dark:border-blue-500', text: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-900/20', icon: <CheckCircle size={16} /> },
    { id: 'Action Required', label: 'Sent Back', count: sentBackCount, color: 'border-t-red-800 dark:border-t-red-600', text: 'text-red-800 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-900/20', icon: <AlertCircle size={16} /> }
  ];
  ...
  return (
    <div className="space-y-4 max-w-full mx-auto text-left animate-in fade-in duration-200 h-[calc(100dvh-100px)] flex flex-col overflow-hidden pb-2 md:pb-4">
      ...
      <div className="flex flex-col md:flex-row gap-4 shrink-0">
        ...
        {/* Profile Card */}
        <div className="flex-1 bg-white dark:bg-[#180e10] p-8 rounded-xl border border-gray-200 dark:border-[#42292f] shadow-sm flex items-start gap-4">
          ...
          <div className="flex flex-col items-start min-w-0">
            <h3 className="text-lg font-black text-gray-900 dark:text-white leading-tight truncate w-full">{profileName || 'Office Processor'}</h3>
            <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2.5 truncate w-full">Processor / {processorOfficeName || 'HRMO'}</p>
            <button 
        ...
        <div className="grid grid-cols-2 md:flex gap-4 shrink-0">
          <div 
            ...
            className="w-full md:w-40 lg:w-90 bg-white dark:bg-[#180e10] p-3 rounded-xl border-t-4 border-t-blue-500 border-x border-b border-gray-200 dark:border-x-[#42292f] dark:border-b-[#42292f] shadow-sm flex flex-col justify-center items-center cursor-pointer hover:shadow-md transition-all active:scale-95 text-center"
            role="button"
            tabIndex={0}
          >
            <div className="flex items-center gap-1.5 md:gap-2 mb-1">
              <Inbox size={16} className="text-blue-500 shrink-0" />
              <span className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider truncate">Incoming Docs</span>
            </div>
            <p className="text-2xl font-black text-blue-600 dark:text-blue-400">{String(expectedIncomingCount || 0).padStart(2, '0')}</p>
            <span className="text-[9px] font-bold text-blue-600 dark:text-blue-400 mt-1 uppercase tracking-tight">Scan Incoming &rarr;</span>
          </div>
          ...
          >
            <div className="flex items-center gap-1.5 md:gap-2 mb-1">
              ...
              <span className="text-[9px] md:text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider truncate">Completed Docs</span>
            </div>
            <p className="text-2xl font-black text-emerald-600 dark:text-emerald-400">{String(completedProcessingCount || 0).padStart(2, '0')}</p>
            <span className="text-[9px] font-bold text-emerald-600 dark:text-emerald-400 mt-1 uppercase tracking-tight">View Archive &rarr;</span>
          </div>
        </div>
      </div>
                ...
                {columnDocs.length > 0 ? (
                  columnDocs.map((doc, idx) => (
                    <div 
                      ...
                      className="bg-white dark:bg-[#180e10] p-3 rounded-lg border border-gray-200 dark:border-[#42292f] shadow-sm hover:shadow-md hover:border-gray-300 dark:hover:border-gray-600 transition-all cursor-pointer group flex flex-col gap-2"
                    >
                      ...
                      <div className="pt-2 border-t border-gray-100 dark:border-gray-800 mt-1 flex items-center justify-between">
                        <span className="text-[10px] text-gray-500 dark:text-gray-400 font-medium">Next:</span>
      ...
      {/* Global Pagination */}
      {totalDashPages > 1 && (
        <div className="bg-white dark:bg-[#180e10] p-2.5 border border-gray-200 dark:border-[#42292f] rounded-xl flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0 shadow-sm">
          <span className="text-xs text-gray-500 dark:text-gray-400 font-medium px-2 text-center sm:text-left">
            Dataset Page <span className="font-bold text-gray-900 dark:text-white">{dashboardPage}</span> of <span className="font-bold text-gray-900 dark:text-white">{totalDashPages}</span>
          </span>
          <div className="flex gap-2 w-full sm:w-auto">
            <button 
              disabled={dashboardPage === 1} 
              onClick={() => setDashboardPage(prev => prev - 1)} 
              className="flex-1 sm:flex-none px-3 py-1.5 sm:py-1 border border-gray-300 dark:border-gray-700 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800 text-xs font-bold text-gray-700 dark:text-gray-300 disabled:opacity-40 transition-colors"
            >
              Prev
            </button>
            <button 
