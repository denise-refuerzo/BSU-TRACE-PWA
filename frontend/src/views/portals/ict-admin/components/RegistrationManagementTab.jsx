...
const statusClass = status => ({
  pending: 'bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-400', active: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-400',
  rejected: 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400', revoked: 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400',
              ...
              <article key={link.link_id} className="rounded-2xl border border-gray-200 dark:border-[#42292f] bg-white dark:bg-[#180e10] p-5 shadow-sm">
                ...
                {link.status === 'active' && (
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button type="button" onClick={() => copyLink(link)} className="inline-flex items-center gap-2 rounded-lg bg-gray-900 dark:bg-gray-800 px-4 py-2 text-xs font-bold text-white hover:bg-black dark:hover:bg-gray-700 cursor-pointer"><Clipboard size={15}/> Copy registration link</button>
                    <button type="button" onClick={() => patchStatus(link,'revoked')} className="inline-flex items-center gap-2 rounded-lg border border-red-200 dark:border-red-900 bg-white dark:bg-[#1c1113] px-4 py-2 text-xs font-bold text-red-700 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 cursor-pointer"><ShieldX size={15}/> Revoke</button>
                  </div>
                )}
                {link.registered_accounts?.length > 0 && (
                  <details className="mt-4 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <summary className="flex cursor-pointer items-center gap-2 p-3 text-sm font-bold text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-[#1c1113]"><Users size={16}/> {link.registered_accounts.length} registered account{link.registered_accounts.length === 1 ? '' : 's'}</summary>
                    <div className="divide-y divide-gray-100 dark:divide-gray-800 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-[#180e10]">
                      {link.registered_accounts.map(account => (
                        <div key={account.userId} className="flex flex-col justify-between gap-1 p-3 text-xs sm:flex-row text-gray-700 dark:text-gray-300">
                          <span><strong>{account.fullName}</strong> · {account.email}</span>
                          <span className="text-gray-500 dark:text-gray-400">{new Date(account.registeredAt).toLocaleString()}</span>
                        </div>
                      ))}
                    </div>
                  </details>
                )}
                {link.registration_path && <p className="mt-3 flex items-center gap-2 break-all text-[10px] text-gray-400 dark:text-gray-500"><Link2 size={12}/>{link.registration_path}</p>}
              </article>
            );
          })}
