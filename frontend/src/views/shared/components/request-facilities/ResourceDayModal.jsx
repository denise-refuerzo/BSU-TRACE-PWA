...
export default function ResourceDayModal({date, facility, bookings, blocks=[], onClose, onRequest, today}) {
  ...
  return <div className="fixed inset-0 z-50 bg-neutral-950/40 backdrop-blur-sm flex items-center justify-center p-4">
    <section role="dialog" aria-modal="true" aria-label="Daily schedule" className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 space-y-5">
      <header className="flex justify-between gap-4"><div><h3 className="text-xl font-bold">{facility === 'Van' ? 'Vehicles' : facility} · {date}</h3><p className="text-sm text-gray-500 mt-1">Approved active bookings occupy their assigned resources. Pending requests do not block a time.</p></div><button onClick={onClose} aria-label="Close daily schedule">✕</button></header>
      <div className="space-y-3">{entries.length ? entries.map(b=>{const active=!['Pending','Reserved','Cancelled','Completed'].includes(b.status);return <article key={b.booking_id} className="rounded-xl border border-gray-200 p-4"><div className="flex justify-between gap-3"><strong className="text-sm">{(b.vr_start||b.gm_start)?.slice(0,5)} – {(b.vr_end||b.gm_end)?.slice(0,5)}</strong><span className={`text-xs rounded-full px-3 py-1 ${active?'bg-emerald-100 text-emerald-800':'bg-amber-100 text-amber-800'}`}>{b.status==='Confirmed'?'Approved':b.status}</span></div><p className="text-sm mt-2">{b.purpose}</p><p className="text-xs text-gray-500 mt-1">{b.full_name}{b.destination ? ` · ${b.destination}` : ''}</p></article>}):<p className="bg-gray-50 rounded-xl p-5 text-sm text-gray-500">No requests submitted for this day.</p>}</div>
