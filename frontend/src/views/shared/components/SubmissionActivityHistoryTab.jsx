  ...
  return (
    <div className="mx-auto max-w-8xl space-y-5 text-left">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <p className="text-xs font-medium text-neutral-500 dark:text-gray-400">A permanent trail of office processing, submission activity, access changes, and document conversations.</p>
        <div className="flex flex-col gap-2 sm:flex-row">
          <label className="relative">
            ...
            <select 
              ...
              className="w-full appearance-none rounded-lg border border-neutral-300 dark:border-gray-700 bg-white dark:bg-[#1c1113] py-2 pl-9 pr-8 text-xs text-neutral-900 dark:text-white outline-none focus:border-red-800 cursor-pointer"
            >
              <option value="all">All activity</option>
              {includeOfficeActivity && <option value="office">Office processing</option>}
              <option value="submission">My submissions</option>
              <option value="access">Collaboration & access</option>
              <option value="chat">Chat activity</option>
            </select>
          </label>
          <label className="relative">
      ...
      <div className="overflow-hidden rounded-2xl border border-neutral-200 dark:border-[#42292f] bg-white dark:bg-[#180e10] shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[860px] text-left text-sm">
            ...
            <tbody className="divide-y divide-neutral-100 dark:divide-gray-800/50">
              {pageRows.map(event => (
                <tr key={`${event.event_source}-${event.event_id}`} className="hover:bg-neutral-50/70 dark:hover:bg-[#2b1317]/40 transition-colors">
                  <td className="p-4 whitespace-nowrap text-xs text-neutral-500 dark:text-gray-400 font-mono">
                    {formatPhilippineDateTime(event.occurred_at)}
                  </td>
                  <td className="p-4">
                    <p className="font-bold text-neutral-900 dark:text-white">{event.title}</p>
                    <p className="mt-1 font-mono text-[10px] text-neutral-500 dark:text-gray-400">{event.qr_code} · {event.relationship_label}</p>
                  </td>
                  <td className="p-4 text-xs font-semibold text-neutral-700 dark:text-gray-300">
                    {event.actor_name}
                  </td>
                  <td className="p-4 text-xs text-neutral-700 dark:text-gray-300">
                    <span className="mb-1 block text-[9px] font-black uppercase tracking-wider text-red-800 dark:text-red-400">{event.event_category}</span>
                    {describe(event)}
                  </td>
                  <td className="p-4 text-right">
                    <button onClick={() => setSelected(event)} className="trace-button dark:bg-[#1c1113] dark:border-gray-700 dark:text-gray-300 dark:hover:bg-[#2b1317]">
                      <Eye size={14}/>View
                    </button>
                  </td>
                </tr>
        ...
        <PaginationControls page={page} totalPages={totalPages} onPageChange={setPage} totalItems={rows.length} pageSize={PAGE_SIZE}/>
      </div>
