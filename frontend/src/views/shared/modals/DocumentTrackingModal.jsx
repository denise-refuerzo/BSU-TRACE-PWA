...
function ActorLine({label,actor}) {
  ...
  return <span className="mt-1.5 flex items-center gap-1 text-[10px] font-medium text-neutral-600"><UserRound size={11} className="shrink-0 text-rose-700"/>{label} <strong className="font-semibold text-neutral-800">{actor.name}</strong></span>;
}
  ...
  return <div className="fixed inset-0 bg-neutral-950/55 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-6">
    <section role="dialog" aria-modal="true" aria-labelledby="document-tracking-title" className="bg-white rounded-2xl w-full max-w-4xl max-h-[92dvh] flex flex-col overflow-hidden text-left shadow-2xl border border-white/30">
      ...
      <div className="trace-dialog-body min-h-0 overflow-y-auto overscroll-contain">{error&&<p role="alert" className="m-5 p-3 rounded-xl border border-rose-200 bg-rose-50 text-sm text-rose-800">{error}</p>}{!doc?<p className="p-6 text-sm text-neutral-500">Loading document…</p>:<div className="p-5 sm:p-6 space-y-5">
        {doc.lifecycle_state==='cancelled'&&<section className="rounded-xl border border-neutral-300 bg-neutral-100 p-4 text-xs text-neutral-800"><div className="font-black uppercase tracking-wide">Processing stopped by submitter</div><p className="mt-2 rounded-lg border border-neutral-200 bg-white p-3 font-medium leading-relaxed">{doc.cancellation_reason||'No reason was recorded.'}</p><p className="mt-2 text-[10px] text-neutral-500">Stopped {formatPhilippineDateTime(doc.cancelled_at)} · retained in the permanent history</p></section>}
        {correctionNotice&&doc.lifecycle_state!=='cancelled'&&<section className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-xs text-rose-950" aria-label="Correction request"><div className="flex items-center gap-2 font-black uppercase tracking-wide"><Undo2 size={15}/>Correction requested</div><p className="mt-2 rounded-lg border border-rose-200 bg-white p-3 font-medium leading-relaxed text-rose-800">{correctionNotice.message||'Corrections are required before processing can continue.'}</p>{correctionNotice.name&&<p className="mt-2 flex flex-wrap items-center gap-x-1.5 gap-y-1 text-[10px] text-rose-800"><UserRound size={12}/><span>Requested by <strong>{correctionNotice.name}</strong>{correctionNotice.office_name?` · ${correctionNotice.office_name}`:''}</span>{correctionNotice.timestamp&&<span>· {formatPhilippineDateTime(correctionNotice.timestamp)}</span>}</p>}</section>}
      ...
      <footer className="shrink-0 px-5 py-4 border-t border-rose-100 bg-[#fffafa] flex flex-wrap justify-between gap-2"><div className="flex flex-wrap gap-2">{doc&&onOpenChat&&doc.lifecycle_state!=='cancelled'&&<button className={button} onClick={()=>{onClose();onOpenChat(doc)}}><MessageSquare size={15}/>Chat regarding this file</button>}{doc&&(doc.permissions?.isOwner||doc.permissions?.isCollaborator)&&<DocumentAccessControls document={doc} onChanged={onRefresh} onCloseParent={onClose}/>} {doc&&<div className="relative" ref={dropdownRef}><button className={button} onClick={()=>setShowExportMenu(v=>!v)}><Download size={14}/>Download QR<ChevronDown size={14}/></button>{showExportMenu&&<div className="absolute left-0 bottom-full mb-2 w-48 bg-white border border-neutral-200 rounded-xl shadow-xl z-50 py-1.5 text-xs font-semibold"><button onClick={handleDownloadImage} className="w-full px-3.5 py-2 text-left hover:bg-neutral-100 flex items-center gap-2"><ImageIcon size={14}/>Image (PNG)</button><button onClick={handlePrintPdf} className="w-full px-3.5 py-2 text-left hover:bg-neutral-100 flex items-center gap-2"><FileText size={14}/>Printable PDF Slip</button></div>}</div>}</div><div className="flex gap-2">{doc&&officeFeatures&&doc.lifecycle_state!=='cancelled'&&<>{mine&&!step.time_in&&<button disabled={busy} className={button+' trace-button-primary'} onClick={()=>act('time-in')}><LogIn size={15}/>Accept</button>}{pending&&<><button disabled={busy} className={button} onClick={()=>{setMode('adhoc');setConfirmation('')}}><GitBranch size={15}/>Ad Hoc</button><button disabled={busy} className={button} onClick={()=>{setMode('return');setConfirmation('')}}><Undo2 size={15}/>Send Back</button><button disabled={busy} className={button+' trace-button-primary'} onClick={()=>{setConfirmation('sign');setMode('')}}><PenLine size={15}/>Sign Document</button></>}{mine&&step.time_in&&[3,4].includes(step.s_id)&&<button disabled={busy} className={button+' trace-button-primary'} onClick={()=>setConfirmation('time-out')}><LogOut size={15}/>{step.s_id===4?'Release for Correction':'Release'}</button>}</>}<button disabled={busy} className={button} onClick={onClose}>Close</button></div></footer>
    </section>
  </div>;
}
