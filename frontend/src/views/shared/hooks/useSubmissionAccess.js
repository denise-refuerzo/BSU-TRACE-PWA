...
export default function useSubmissionAccess(userId) {
  const [access, setAccess] = useState({ offices: [], departments: [], canRequestRegistration: false });
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      if (response.ok) setAccess({ offices: data.offices || [], departments: data.departments || [], canRequestRegistration: Boolean(data.canRequestRegistration) });
    } finally {
      setLoading(false);
    }
  }, []);
